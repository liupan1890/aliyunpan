...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: getAsarPath('dist/main/preload.js')
    }
  })
  mainWindow.removeMenu()
  if (DEBUGGING) {
    const url = `http://${process.env['VITE_DEV_SERVER_HOST']}:${process.env['VITE_DEV_SERVER_PORT']}/index.html`
    mainWindow.loadURL(url, {
      userAgent: ua,
      httpReferrer: Referer
    })
  } else {
    mainWindow.loadURL('file://' + getAsarPath('dist/renderer/main.html'), {
      userAgent: ua,
      httpReferrer: Referer
    })
  }
  ...
  mainWindow.on('ready-to-show', function () {
    mainWindow!.webContents.send('setPage', { page: 'PageMain' })
    mainWindow!.webContents.send('setTheme', {
...
function creatWorker() {
  ...
  if (DEBUGGING) {
    ...
    workerWindow.loadURL('file://' + getAsarPath('dist/renderer/main2.html'), {
      userAgent: ua,
      httpReferrer: Referer
    })
  }
  ...
  workerWindow.on('ready-to-show', function () {
    workerWindow!.webContents.send('setPage', { page: 'PageWorker' })
  })

  if (isDbu || DEBUGGING) {
    //workerWindow.setSize(800, 600)
    //workerWindow.show()
...
function creatAria() {
  try {
    ...
    const options = { detached: true, stdio: 'ignore', cwd: basePath }
...
app
  ...
  /*
  .then(() => {
    ShowError('getAppPath', app.getAppPath())
    ShowError('__dirname', __dirname)
  })
  */
  .then(() => {
...
ipcMain.on('WebOpenWindow', (event, data) => {
  ...
  if (DEBUGGING) {
    ...
    win.loadURL('file://' + getAsarPath('dist/renderer/main2.html'), {
      userAgent: ua,
      httpReferrer: Referer
    })
  }
