...
async function ApiBatchDirFileList(user_id: string, drive_id: string, dirlist: IAliDirBatchResp[], ScanType: string) {
  ...
  const url = 'v2/batch?jsonmask=responses(id%2Cstatus%2Cbody(next_marker%2Ctotal_count%2Citems(name%2Cfile_id%2Cdrive_id%2Ctype%2Csize%2Cupdated_at%2Ccategory%2Cfile_extension%2Cparent_file_id%2Cmime_type%2Cmime_extension%2Ccontent_hash%2Cpunish_flag)))'
...
async function ApiWalkDirFileList(user_id: string, drive_id: string, file_id: string, limit: number, category: string) {
  ...
  let items: any[] = []
  do {
    let url = 'v2/file/walk?jsonmask=next_marker%2Citems(name%2Cfile_id%2Cdrive_id%2Ctype%2Csize%2Cupdated_at%2Ccategory%2Cfile_extension%2Cparent_file_id%2Cmime_type%2Cmime_extension%2Ccontent_hash%2Cpunish_flag)'
    let postdata = {
      all: false,
      drive_id: drive_id,
    ...
    try {
      ...
      DebugLog.mSaveLog('danger', 'ApiWalkDirFileList' + file_id + ' error=' + (e.message || ''))
      break 
    }
  } while (next_marker)
  return items
