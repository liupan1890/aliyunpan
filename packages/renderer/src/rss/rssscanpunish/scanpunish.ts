      ...
      for (let i = 0, maxi = dirlist.length; i < maxi; i++) {
        if (dirlist[i].next_marker && dirlist[i].items.length < 2000) {
          ...
          if (weifa > 0) {
            ...
            while (true) {
              if (!dir.parent_file_id || PanData.WeiGuiDirMap.has(dir.parent_file_id)) break
          ...
          if (noshare > 0) {
            ...
            while (true) {
              if (!dir.parent_file_id || PanData.NoShareDirMap.has(dir.parent_file_id)) break
...
async function ApiBatchDirFileList(user_id: string, drive_id: string, dirlist: IAliDirBatchResp[], ScanType: string) {
  ...
  const url = 'v2/batch?jsonmask=responses(id%2Cstatus%2Cbody(next_marker%2Ctotal_count%2Citems(name%2Cfile_id%2Cdrive_id%2Ctype%2Csize%2Cupdated_at%2Ccategory%2Cfile_extension%2Cparent_file_id%2Cmime_type%2Cmime_extension%2Cpunish_flag)))'
