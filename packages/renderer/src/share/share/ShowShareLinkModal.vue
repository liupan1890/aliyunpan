...
export default defineComponent({
  ...
  setup(props) {
    ...
    return { okLoading, handleOpen, handleClose, TreeHeight, treeref, handleTreeSelect, share, sharetoken, expiration, dayjs, TreeData, TreeExpandedKeys, TreeSelectedKeys, TreeCheckedKeys, onLoadData, file_list }
  },
  methods: {
    handleHide() {
      modalCloseAll()
...
async function SaveLink(savetype: string, shareid: string, sharetoken: string, user_id: string, drive_id: string, parentid: string, nodes: CheckNode[]) {
  ...
  for (let i = 0, maxi = halfNodes.length; i < maxi; i++) {
    ...
    if (file_id != undefined && file_id != 'QuotaExhausted.Drive') {
      ...
      const rchild = await SaveLink(savetype, shareid, sharetoken, user_id, drive_id, file_id, half.children)
...
<template>
  <a-modal :visible="visible" modal-class="modalclass showsharemodal" @cancel="handleHide" @before-open="handleOpen" @close="handleClose" title-align="start" :footer="false" :unmount-on-close="true" :mask-closable="false">
    ...
    <div v-if="withsave" class="modalfoot">
