...
import { ArrayToMap, HanToPin } from '@/utils/utils'
  ...
  actions: {
    ...
    mSaveDirFileLoading(drive_id: string, dir_id: string, dir_name: string) {
      ...
      if (this.DirID != dir_id || this.DriveID != drive_id) {
        this.$patch({ DriveID: drive_id, DirID: dir_id, DirName: dir_name, ListOrderKey: order, ListLoading: true, ListLoadingIndex: 0, ListSearchKey: '', ListDataRaw: [], ListDataShow: [], ListSelected: new Set(), ListFocusKey: '', ListSelectKey: '' })
      } else {
        
        this.$patch({ DriveID: drive_id, DirID: dir_id, DirName: dir_name, ListOrderKey: order, ListLoading: true, ListLoadingIndex: 0, ListSearchKey: '', ListDataRaw: [], ListDataShow: [] })
      }
    },
    
    mSaveDirFileLoadingPart(pageindex: number, partdir: IAliFileResp) {
    ...
    mSaveDirFileLoadingFinish(drive_id: string, dir_id: string, list: Item[]) {
      ...
      //todo:: 自定义排序
      //this.ListDataRaw = this.mGetOrder(this.ListOrderKey, freezelist)
    ...
    mGetOrder(order: string, list: Item[]) {
      //todo:: 其他排序方式
      if (order == 'size') list.sort((a, b) => b.size - a.size) 
      if (order == 'time') list.sort((a, b) => b.time - a.time) 
      return list
    },
    ...
    mDeleteFiles(dir_id: string, fileidlist: string[]) {
