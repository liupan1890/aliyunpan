  ...
  setup(props) {
    ...
    const makeImageSheBei = (exif: string | undefined) => {
      ...
      try {
        ...
        DebugLog.mSaveLog('warning', exif)
      }
      return ''
    }
    ...
    return { okLoading, handleOpen, handleClose, fileinfo, dirinfo, dirpath, humanDateTimeDateStr, humanSize, humanTime, makeFenBianLv, makeImageSheBei, makeImageShiJian }
  },
  methods: {
    handleHide() {
      modalCloseAll()
    ...
    handleCopyDownload() {
      if (this.fileinfo?.download_url) {
        copyToClipboard(this.fileinfo?.download_url)
  ...
  <a-modal :visible="visible" modal-class="modalclass shuxingmodal" @cancel="handleHide" @before-open="handleOpen" @close="handleClose" :footer="false" :unmount-on-close="true" :mask-closable="false">
    ...
    <div class="modalbody" style="width: 500px; max-height: calc(80vh - 100px); overflow-y: scroll">
      ...
      <div v-if="fileinfo?.category == 'audio'">
        <div class="h16"></div>
        <div width="100%">
          <audio controls style="width: 100%; height: 32px" :src="fileinfo?.thumbnail">您的浏览器不支持 audio 元素</audio>
        </div>
      </div>
    ...
    <div class="modalfoot">
      ...
      <a-button type="outline" size="small" @click="handleCopyDownload">复制下载链接</a-button>
    </div>
  </a-modal>
</template>
