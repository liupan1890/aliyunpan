...
import { WebviewTag as Webview } from 'electron'
...
export default defineComponent({
  setup() {
    ...
    function loginbizExt(msg: string) {
      ...
      if (!data.bizExt) {
        DebugLog.mSaveLog('danger', '登录失败：' + msg)
        return
      }
      w(data.bizExt).then((jsonstr: string) => {
        try {
          ...
          DebugLog.mSaveLog('danger', '登录失败：' + (e.message || '解析失败'))
          DebugLog.mSaveLog('danger', JSON.stringify(e))
        }
      })
    }
