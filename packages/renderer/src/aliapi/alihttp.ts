  ...
  static CatchError(error: any, token: ITokenInfo | undefined) {
    try {
      ...
      if (error.response) {
        ...
        if (islog) DebugLog.mSaveLog('warning', 'HttpError4 status=' + error.response.status + ' code=' + error.response.data?.code + ' message=' + errormessage)
        ...
        if (error.code == 'ERR_NETWORK' || (error.response.status == 0 && !error.response.headers)) {
          DebugLog.mSaveLog('warning', 'HttpError0 message=' + errormessage)
        ...
        DebugLog.mSaveLog('warning', 'HttpError1 message=' + errormessage)
        return { code: 601, header: '', body: 'NetError ' + errormessage }
      } else if (error.message) {
        DebugLog.mSaveLog('warning', 'HttpError3 status=' + error.response?.status || '' + ' message=' + errormessage)
        return { code: 603, header: '', body: 'NetError ' + errormessage }
      } else {
        
        DebugLog.mSaveLog('warning', 'HttpError2 message=' + errormessage)
        return { code: 602, header: '', body: 'NetError ' + errormessage }
      }
    } catch (e: any) {
      
      DebugLog.mSaveLog('warning', 'HttpError5  message=' + (e.message || ''))
